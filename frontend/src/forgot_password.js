...
export default function ForgotPassword() {
  ...
  const hadnleEmailSubmit = async () => {
    axios.get(`http://localhost:8080/user/change/generateToken?email=${email}`)
    .then((res) => {
  ...
  const handlePasswordSubmit = async () => {
    axios.get(`http://localhost:8080/user/change/resetPassword?email=${email}&token=${otp}&password=${password}`)
    .then((res) => {
        console.log(res.data)
        handleNext()
        setError(null)
