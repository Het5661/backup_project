...
export default function EditAssignment({course, created_assignment}) {
  ...
  const handleDeleteClick = (id) => () => {
    ...
    axios.delete(`http://localhost:8080/assignment/${delete_row._id}`,{headers:{'Authorization':get('token')}})
  ...
  const processRowUpdate = (newRow) => {
    ...
    axios.put(`http://localhost:8080/assignment/${updatedRow._id}`,{
  ...
  // Added
  useEffect(() => {
    axios.get(`http://localhost:8080/assignment/course/${course._id}`,{headers:{'Authorization': get('token')}})
    .then((resp)=>{   // if no error
