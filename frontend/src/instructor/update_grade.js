...
export default function UpdateGrade({assignmentId}) {
  ...
  const processRowUpdate = (newRow) => {
    ...
    axios.put(`http://localhost:8080/submission/grade/${updatedRow.id}`,{
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/submission/assignment/${assignmentId}`,{headers:{'Authorization': get('token')}})
    .then((resp)=>{   // if no error
