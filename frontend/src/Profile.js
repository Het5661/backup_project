...
function DashboardContent({setIsLoggedIn,navigate,user }) {
  ...
  const handleSave = () => {
    axios.put(`http://localhost:8080/user/${user.user._id}`,{
      username : username,
      email : email,
      phone : phone
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/user/${user.user._id}`,{headers:{'Authorization':get('token')}})
    .then((resp)=>{   // if no error
      console.log(resp);
  ...
  const handleSubmitChangePassword = async (e) => {
    ...
    axios.put(`http://localhost:8080/user/change/password`,{
