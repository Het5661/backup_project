...
const Announcement = ({course }) => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/announcement/course/${course._id}`,{headers:{'Authorization': get('token')}})
  ...
  const handleClickOpenComment = async (id) => {
    ...
    axios.get(`http://localhost:8080/comment/announcement/${id}`,{headers:{'Authorization':get('token')}})
      .then((resp)=>{   // if no error
  ...
  const handleEditAnnouncementSave = async () => {
    axios.put(`http://localhost:8080/announcement/${announcementEdit._id}`,{
      title : title,
  ...
  const handleSave = () => {
    axios.post(`http://localhost:8080/announcement/course/${course._id}`,{
      title : title,
      description : description,
      files : [],
      comments : []
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:8080/announcement/${id}`,{headers:{'Authorization':get('token')}})
    .then((resp)=>{   // if no error
  ...
  const handleCommentSubmit = async () => {
    ...
    console.log(comment);
    axios.post(`http://localhost:8080/comment/announcement/${announcementId}`,{
      description : comment,
    },{headers:{'Authorization':get('token')}})
