...
const Material = ({announcements, course, instructor }) => {
    ...
    useEffect(() => {
      axios.get(`http://localhost:8080/course/material/${course._id}`,{headers:{'Authorization': get('token')}})
  ...
  const handleSave = () => {
    ...
    axios.post(`http://localhost:8080/course/material/${course._id}`,{
      filename : title,
      data : item,
      description : description
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:8080/course/material/${id}?course_id=${course._id}`,{headers:{'Authorization':get('token')}})
    .then((resp)=>{   // if no error
  ...
  const handleClickOpenComment = async (id) => {
    ...
    axios.get(`http://localhost:8080/comment/file/${id}`,{headers:{'Authorization':get('token')}})
      .then((resp)=>{   // if no error
  ...
  const handleCommentSubmit = async () => {
    ...
    console.log(comment);
    axios.post(`http://localhost:8080/comment/file/${announcementId}`,{
      description : comment,
    },{headers:{'Authorization':get('token')}})
