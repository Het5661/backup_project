...
export default function EnrolledStudentList({course}) {
  ...
  React.useEffect(() => {
    axios.get('http://localhost:8080/course/user/' + course._id,
  ...
  const onDelete = (id) => {
    axios.delete('http://localhost:8080/course/user/'+ id + '?courseId=' + course._id,{headers:{'Authorization': get('token')}})
      .then(response => {
